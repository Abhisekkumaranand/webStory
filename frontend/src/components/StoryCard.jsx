...
export default function StoryCard({ story, to, onOpen, className = "" }) {
  ...
  const content = (
    ...
    >
      ...
      <div className="w-full h-48 md:h-40 bg-gradient-to-br from-gray-100 to-gray-200 overflow-hidden relative">
        {thumb ? (
          <img
            src={thumb}
            alt={`${title} thumbnail`}
        ...
        {/* Slide count */}
        <div className="absolute top-3 right-3 bg-black/60 text-white text-xs rounded-full px-2 py-1 flex items-center gap-1.5 backdrop-blur-sm">
          <svg
            className="w-3 h-3"
            viewBox="0 0 24 24"
            fill="currentColor"
      ...
      <div className="p-4 md:p-5">
        <h3
          ...
          className="text-base md:text-lg font-bold leading-tight text-gray-900 truncate mb-2"
        >
          {title}
        </h3>
